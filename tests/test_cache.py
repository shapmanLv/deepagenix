...
class TestCacheImplementations:
    class BasicOperationsTests(BaseCacheTestCase):
        def test_basic_operations(self):
            ...
            self.assertEqual(self.cache_client.get('key1'), 'value1')  # 验证获取值是否正确
        ...
        def test_ttl_expiration(self):
            ...
            self.assertEqual(self.cache_client.get('temp_key'), 'temp_value')  # 验证缓存设置成功
            sleep(1.1)  # 等待超过TTL时间
    ...
    class RedisSpecificTests(BaseCacheTestCase):
        ...
        def test_redis_serialization(self):
            ...
            self.assertEqual(self.cache_client.get('obj_key'), test_obj)  # 验证获取到的对象正确
        ...
        def test_unserializable_data(self):
            if isinstance(self.cache_client, MemoryCache):
                ...
                self.assertTrue(callable(self.cache_client.get('lambda')))  # 验证获取的对象是可调用的
            else:
                # Redis 不能处理不可序列化的对象（如 lambda 函数）
                with self.assertRaises(Exception):
                    self.cache_client.set('lambda', lambda x: x+1)  # 存储 lambda 函数，预期抛出异常
...
def load_tests(loader, tests, pattern):
    ...
    for client_cls in [MemoryCacheTestCase, RedisCacheTestCase]:
        for test_class in [TestCacheImplementations.BasicOperationsTests,
                         TestCacheImplementations.RedisSpecificTests]:
            tests = loader.loadTestsFromTestCase(test_class)
            for test in tests:
                test.client_type = client_cls.client_type  # 设置当前测试的缓存客户端类型
